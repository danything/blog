...
実例がdenpaのチューナーエージェントです。電波を掴んで素のTSを流すだけの部品なのですが、最初はbunで書いていたものを計測したうえで.NETに書き直しています。Native AOTで実行ファイル1個、依存なしです。  
理由は大きく2つで、1つは接続の扱いです。チューナーを取り上げられた等の失敗時には受け取る側に正常終了として届かないよう接続ごと壊す必要があるのですが、bun版は接続を壊せず、尻切れの録画が「録れた」ことになっていました。もう1つはリアルタイム性で、カーネル側のdvrのリングバッファは既定で1.8MB、地上波の18Mbit/秒だと0.9秒分しかなく、読む側がGC等で一瞬止まるだけで溢れます。この辺りは実機で回数を測りながら詰めていて、ioctlを直に叩けるのも.NET版だけだったので、そちらに落ち着きました。詳細は[agent.md](https://github.com/DAnything/denpa/blob/main/docs/agent.md)に測った数字ごと書いてあります。

要するに画面とAPIはSvelteKit、機材やリアルタイムに近いところや重い計算は.NET、という切り分けです。
